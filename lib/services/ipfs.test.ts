...
import { uploadArrayToIpfs, getIpfsHash, pinClaimsListToIpfs, type Balance } from './ipfs'
...
describe('IPFS Service', () => {
  ...
  describe('getIpfsHash', () => {
    ...
    it('should handle fetch errors', async () => {
      ...
      await expect(getIpfsHash('QmTestHash123')).rejects.toThrow('Fetch failed')
    })
  })
  ...
  describe('pinClaimsListToIpfs', () => {
    ...
    it('should handle duplicate pins', async () => {
      ...
      expect(result).toBe('QmTestHash123')
      expect(consoleSpy).toHaveBeenCalledWith('Duplicate pin detected: ', 'QmTestHash123')
    })
    ...
    it('should handle pinning errors', async () => {
      ...
      await expect(pinClaimsListToIpfs(mockBalances)).rejects.toThrow('Pinning failed')
      expect(consoleSpy).toHaveBeenCalledWith('IPFS upload error: ', expect.any(Error))
    })
